...
def colorize_image(image, color):
    colored_surface = pygame.Surface(image.get_size())
    colored_surface.fill(color)
    
    colored_surface.set_colorkey((0, 0, 0)) 
    colored_surface.blit(image, (0, 0), special_flags=pygame.BLEND_MULT)
...
def handle_vertical_collision(player, objects, dy):
    ...
    for obj in objects:
        if pygame.sprite.collide_mask(player, obj):
            if dy > 0:  # Va chạm từ trên xuống
                player.rect.bottom = obj.rect.top
                player.landed()
                player.y_vel = 0
            elif dy < 0:  # Va chạm từ dưới lên
